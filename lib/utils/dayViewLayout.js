...
function startsBefore(date, min) {
  return _dates2.default.lt(date, min, 'minutes');
}

function positionFromDate(date, min, total) {
  if (startsBefore(date, min)) return 0;

  var diff = _dates2.default.diff(min, date, 'minutes');
...
var isSibling = function isSibling(idx1, idx2, _ref2) {
  ...
  var startDifference = Math.abs(start1 - start2);
  var timeslotSize = step * timeslots;
  var isSmallerThanTimeslot = startDifference < timeslotSize;
  var secondStartsBeforeFirstEnds = start2 < end1;
  var sameStartAsEnding = start1 === start2 && start2 === end1;
